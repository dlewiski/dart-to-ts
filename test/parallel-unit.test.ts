  ...
  async analyzeFunctionality(chunks: any[]): Promise<any> {
    ...
    for (let i = 0; i < chunks.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (chunk, index) => {
        ...
        if (this.simulateErrors && chunk.files[0]?.content === 'TRIGGER_ERROR') {
...
async function runUnitTests() {
  ...
  try {
    ...
    const chunks = Array(6).fill(0).map((_, i) => ({
      category: 'test',
    ...
    if (duration < expectedTime + tolerance && duration >= expectedTime - tolerance) {
  ...
  try {
    ...
    if (progressEvents.length > 0) {
      ...
      if (lastEvent.percentage === 100 && lastEvent.processed === 1) {
        console.log(`  ✅ Progress events working (${progressEvents.length} events)\n`);
        passed++;
      } else {
  ...
  try {
    ...
    const chunks = Array(5).fill(0).map((_, i) => ({
      category: 'test',
      files: [{ content: `chunk${i}` }]
    }));
    ...
    if (maxConcurrent <= maxWorkers) {
      console.log(`  ✅ Concurrency limit respected (max: ${maxConcurrent}/${maxWorkers})\n`);
      passed++;
    } else {
      throw new Error(`Exceeded concurrency limit: ${maxConcurrent} > ${maxWorkers}`);
    }
  ...
  try {
    ...
    const chunks = Array(100).fill(0).map((_, i) => ({
