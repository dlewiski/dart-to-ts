...
program
    ...
    try {
        ...
        const files = await Promise.all(dartFilePaths.map(async (filePath) => {
        ...
        await fs.writeJSON(path.join(config.decisionsPath, 'conversion-decisions.json'), decisions, { spaces: 2 });
        // Generate reports
...
async function analyzeDryRun(files, _config) {
