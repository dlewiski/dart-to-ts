...
function extractParts(content: string): string[] {
  const parts: string[] = [];
...
function extractLibraryName(content: string): string | undefined {
...
function printAnalysisSummary(summary: any, verbose: boolean) {
  ...
  if (summary.packages) {
    console.log(chalk.white('\nPackage Analysis:'));
    const packagesByAction = summary.packages.reduce((acc: any, pkg: any) => {
  ...
  if (summary.techDebt) {
    console.log(chalk.white('\nTechnical Debt:'));
    const debtBySeverity = summary.techDebt.reduce((acc: any, debt: any) => {
